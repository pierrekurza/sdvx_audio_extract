...
def convert_audio_and_move_file(folder_path: str, folder_number: int, output_path: str, music_name: str,
    ...
    for music in list_of_files:
        if not re.search("(_pre.s3v$)", music):
            music_path = music
    # Launch extract
    output_path_final = os.path.join(output_path, album_name, str(folder_number), ascii_name, ".mp3")
...
def main():
    ...
    covers_dir = glob.glob(extract_folder + "\\covers" + "\\*")
    for i in covers_dir:
        os.remove(i)
    if os.path.isdir(os.path.join(extract_folder, "covers")):
        os.rmdir(extract_folder + "\\covers")
